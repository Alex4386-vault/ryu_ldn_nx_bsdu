...
bool BsdMitmService::ShouldMitm(const sm::MitmProcessInfo& client_info) {
    ...
    constexpr u64 OUR_PROGRAM_ID = 0x4200000000000667ULL;
    ...
    const sf::InAutoSelectBuffer& addr)
{
    ...
    out_errno.SetValue(errno_out);
    if (R_SUCCEEDED(rc) && errno_out == 0 &&
        addr.GetSize() >= sizeof(ryu_ldn::bsd::SockAddrIn) && addr.GetPointer() != nullptr) {
        ryu_ldn::bsd::SockAddrIn sa{};
        __builtin_memcpy(&sa, addr.GetPointer(), sizeof(sa));
        lsp_capture::OnBind(fd, sa.GetPort());
    }
    R_RETURN(rc);
}
    ...
    sf::OutAutoSelectBuffer addr_out)
{
    ...
    if (R_SUCCEEDED(rc) && out.errno_val == 0 && out.new_fd >= 0) {
        lsp_capture::OnAccept(fd, out.new_fd);
    }
    R_RETURN(rc);
}

/**
 * @brief Get peer name (Command 15)
 *
    ...
    const sf::InAutoSelectBuffer& buffer)
{
    ...
    if (R_SUCCEEDED(rc) && out.errno_val == 0 && out.size > 0) {
        lsp_capture::LogTcp(fd, lsp_capture::Dir::Tx, buffer.GetPointer(),
    ...
    const sf::InAutoSelectBuffer& addr)
{
    ...
    if (R_SUCCEEDED(rc) && out.errno_val == 0 && out.size > 0) {
        ...
        lsp_capture::LogUdp(fd, lsp_capture::Dir::Tx, buffer.GetPointer(),
                            static_cast<size_t>(out.size), rport);
    }
    R_RETURN(rc);
}
    ...
    sf::OutAutoSelectBuffer buffer)
{
    ...
    out_size.SetValue(out.size);
    if (R_SUCCEEDED(rc) && out.errno_val == 0 && out.size > 0) {
        lsp_capture::LogTcp(fd, lsp_capture::Dir::Rx, buffer.GetPointer(),
                            static_cast<size_t>(out.size));
    }
    R_RETURN(rc);
}
    ...
    sf::OutAutoSelectBuffer addr_out)
{
    ...
    if (R_SUCCEEDED(rc) && out.errno_val == 0 && out.ret > 0) {
        lsp_capture::LogUdp(fd, lsp_capture::Dir::Rx, buffer.GetPointer(),
                            static_cast<size_t>(out.ret), 0);
    }
    R_RETURN(rc);
}
